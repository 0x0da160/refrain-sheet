...
| CSV → RSF conversion / full value read, 200,000×6 cells            | every value read built a `FieldNode`, a `subarray` view and a cache entry and ran two `TextDecoder` calls (display + strict check) per field | short ASCII fields are built directly from the bytes, value-only reads skip the node cache, and the strict check runs only for a file with undecodable bytes; `RsfDocument.fromLossless` dropped from ~1,424 ms to ~386 ms mean (~3.7×), measured with `npm run bench` on this change's cloud runner (not the pinned Docker/Windows reference environment above)                                                                                                     |
