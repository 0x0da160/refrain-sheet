...
| 確定してセルに留まる       | `Ctrl + Enter`                                        | 編集中のみ。複数セルへの一括入力はしない                                                                                                                                     |
| 範囲を同じ値で埋める       | —                                                     | `Ctrl + Enter`                                                                                                                                                               | —                                                | セル内改行・入力確定との区別              |
| 下／右方向にコピー         | —（Web セクションで確認できず）                       | `Ctrl + D`／`Ctrl + R`                                                                                                                                                       | —                                                | `Ctrl + R` はブラウザーの再読み込み       |
| 値だけ貼り付け             | `Ctrl + Shift + V`（設定で切り替え）                  | 既定は値のみ貼り付け（Google と同じ意味、オーナー決定）。設定で書式のみ貼り付け（Excel for the web と同じ意味）に変更できる。どちらも「編集 > 形式を選択して貼り付け」にある |
| 書式を貼り付け             | `Ctrl + Shift + V`                                    | —（同じキーは「値だけ」）                                                                                                                                                    | —                                                | **同じキーで意味が異なる**                |
| 今日の日付／現在時刻       | `Ctrl + ;`／`Ctrl + Shift + ;`                        | `:` の文字で判定するため、日本語配列では時刻が `Ctrl + :`。日本語配列の `Ctrl + Shift + ;`（`+`）はブラウザーの拡大のまま。`2026-09-25`／`13:45` の文字列を入力              |
...
| 次／前のシート       | `Ctrl + Alt + PageDown`／`PageUp`                                 | Excel for the web と同じ。`Ctrl + PageDown`／`PageUp` はブラウザーのタブ移動、Google の `Alt + ↓`／`↑` は採用しない。以前の `F7`／`Shift + F7` は廃止 |
| 太字／斜体／下線     | `Ctrl + B`／`I`／`U`                                              | 採用済み                                                                                                                                              |
| 切り取り             | `Ctrl + X`                                                        | 3 製品と同じキー。コピーしたうえでセルをすぐ空にする（1回の元に戻すで戻る）。参照を保ったまま動かすのは「選択範囲の移動」                             |
...
| 参照形式の切り替え   | 数式入力中の `F4`                                                 | 3 製品と同じ。数式入力中以外の `F4` には何も割り当てない                                                                                              |
...
| データ領域の端へ移動 | `Ctrl + 方向キー`（`Shift` を加えると選択）                       | Excel・Calc と同じ。フィルターで隠れた行は飛ばす                                                                                                      |
| 1画面分の移動        | `PageUp`／`PageDown`                                              | 表示中の行数ぶん移動（以前は固定 20 行）                                                                                                              |
| 今日の日付／現在時刻 | 未実装                                                            | 入力コマンド自体がないため、機能追加として別途検討する                                                                                                |
| リボン・アクセスキー | 採用しない                                                        | リボンがなく、Excel 固有の操作体系を再現しない（IP リスク方針）                                                                                       |

注意：以前は `F4` を「新規作成」、`F7`／`Shift + F7` をシート切り替え、`F8` を
「タブを閉じる」に割り当てていたが、他製品と意味が違うため 2026年9月26日に
外した。新規作成とタブを閉じるはメニューから行う（本来のキー `Ctrl + N`／
`Ctrl + W` はブラウザーが使う）。
