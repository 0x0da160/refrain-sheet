...
//
//   node design-system/v2/tools/build.mjs          write every output
//   node design-system/v2/tools/build.mjs --check  verify outputs are current
//
...
function buildFoundationsCss() {
  ...
  for (const [name, v, note] of F.focus) L.push(decl(name, v, note));
  L.push('');
  L.push('  /* ---------- target sizes ---------- */');
  for (const [name, px, note] of F.targets) L.push(decl(name, rem(px), `${px}px — ${note}`));
  L.push('');
  L.push('  /* ---------- icons (Lucide) ---------- */');
  for (const [name, px, note] of F.iconSizes) L.push(decl(name, rem(px), `${px}px — ${note}`));
  L.push(decl('icon-stroke', `${F.iconStroke}px`, 'Rendered stroke at every size'));
  L.push('');
  L.push('  /* ---------- motion ---------- */');
...
function buildBrandCss() {
  ...
  for (const row of B.typeScale) {
    ...
    L.push(decl(`brand-leading-${name}`, `var(--leading-${lh})`, `${leading(lh)}`));
...
function dtcgFoundations() {
  ...
  return {
    ...
    target: group('dimension', F.targets, 'rem', (px) => px / 16, (n) => n.replace('target-', '')),
...
function dtcgDensity(i) {
  ...
  for (const [name, v] of Object.entries(density)) size[name] = { $value: dim(v[i] / 16, 'rem'), $description: v[3] };
...
function dtcgBrand() {
  ...
  for (const [name, min, , max, lh, weight, role] of B.typeScale) {
    typography[name] = {
      $value: { fontFamily: '{font.family.ui}', fontSize: dim(max, 'rem'), fontWeight: weight === 700 ? '{font.weight.bold}' : '{font.weight.regular}', letterSpacing: dim(0, 'px'), lineHeight: leading(lh) },
...
function docFoundationScales() {
  return [
    ...
    scaleTable('ターゲットの大きさ', F.targets.map(([n, px, note]) => [`--${n}`, px, note]), (r) => `${r[1]}px`),
...
function docFonts() {
  const roles = { 'font-ui': 'アプリの UI、LP の本文と見出し', 'font-data': 'セル・数値・パス（アプリの既定のスプレッドシートのフォント）', 'font-code': 'コード・JSON と YAML のエディタ・ショートカット表記・LP の見出し上の英字ラベル' };
  return table(['トークン', '用途', 'スタック'], Object.entries(F.fonts).map(([name, list]) => `<tr><th><code>--${name}</code></th><td>${esc(roles[name] ?? '')}</td><td><code>${esc(family(list))}</code></td></tr>`));
}
function docAppType() {
...
const manifest = {
  ...
  layers: {
    ...
    brand: { purpose: 'Landing site, documentation, store listings', requires: 'foundations', css: 'brand/css/refrain-brand.css', docs: 'brand/docs/brand-guidelines.html' },
  },
  source: 'tools/source/*.mjs',
  build: 'node design-system/v2/tools/build.mjs [--check]',
...
if (stale) {
  process.stderr.write('Run: node design-system/v2/tools/build.mjs\n');
  process.exit(1);
}
