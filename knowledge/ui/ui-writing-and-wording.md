...
対象ごとに個別の Issue／PR で行い（表の Issue 列）、§4 の最後の段落に従って
メニュー・ダイアログ・ヘルプを同時に揃える。

| キー                                               | 現行の日本語                                            | 本ルールとの差分                                                               | Issue                                                        |
| -------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------ |
| `dialog.unsaved.title`                             | 未保存の変更                                            | 見出しが判断事項になっていない（§2-7、§5.1）                                   | [#636](https://github.com/0x0da160/refrain-sheet/issues/636) |
| `dialog.unsaved.discard` / `dialog.unsaved.cancel` | 破棄 / キャンセル                                       | 選択後の動作が具体的でない（「変更を破棄」「編集を続ける」）（§2-7）           | [#636](https://github.com/0x0da160/refrain-sheet/issues/636) |
| `dialog.unrepresentable.message`                   | 選択した文字コード（{encoding}）では表現できない文字が… | 対象セルは別行で示されるが、次の行動が書かれていない（§2-6、§5.2）             | [#637](https://github.com/0x0da160/refrain-sheet/issues/637) |
| `notify.saveFailed`                                | 保存に失敗しました: {error}                             | 対象と次の行動がなく、技術的なエラー文だけになり得る（§2-6）                   | [#638](https://github.com/0x0da160/refrain-sheet/issues/638) |
| `notify.savedDownload`                             | ダウンロードとして「{name}」を保存しました。…           | ダウンロードの完了を観測できているかを確認し、観測範囲に合わせる（§2-8、§5.3） | [#639](https://github.com/0x0da160/refrain-sheet/issues/639) |
| `dialog.sheetName.ok` / `dialog.ok`                | OK                                                      | 対象＋動詞にできるか確認する（§2-1）                                           | [#640](https://github.com/0x0da160/refrain-sheet/issues/640) |
| `diag.text-after-quote` / `find.invalidRegex` など | 「不正な」「不正です」                                  | 利用者を責める印象を避け、問題と次の行動を書けるか確認する（§2-2、§2-6）       | [#641](https://github.com/0x0da160/refrain-sheet/issues/641) |
